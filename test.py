...
import json,requsts,LineService
from thrift.transport import THttpClient
#import pyimgflip

cl = LineClient(authToken='EL5Ad7YfGsmppMeJh5s0.INRWROiZvFmws/gcCCwy4a.mH6RnBLjDAQWn8tUbdpLkD3+KwxtKOnRA9exwsol8qQ=')
...
owner = ["u8a7af0ae823355f3164be18271c598e0"]
admin = ["u8a7af0ae823355f3164be18271c598e0"]
staff = ["u8a7af0ae823355f3164be18271c598e0"]
...
Headers3 = {
        ...
        'X-Line-Application': "CHROMEOS 1.7.14BotEater x64",
        "x-lal": "ja-US_US",
    }
nama2 = 'WIN'
Headers = {
        'User-Agent': "Line\8.3.3",
        'X-Line-Application': "DEKSTOPWIN\8.3.0RFU-BOT\t18.99",
        "x-lal": "ja-US_US",
    }
nama3 = 'ios'
Headers5 = {
        'User-Agent': "Line\8.0.0",
        'X-Line-Application': "IOSIPAD\t7.18.0tiPhoneOS\t11.12.1",
        "x-lal": "ja-US_US",
    }
...
#KICKALL
                          ...
                          if msg._from in admin:
                           if msg.toType == 2:
                              print("ok")
                              _name = msg.text.replace("!curut","cannibal")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               for target in targets:
                                 ...
                                 separate = msg.txt.split( " ")
                                 jmlh = int(separate[1])
                                 for x in range(jmlh):
                                     Header3.update({'x-lpqs' : ' /api/v4/TalkService.do'})
                                     transport = THttpClient.THttpClient('https://gd2.line.naver.jp/v4/Talk...)
                                     transport.setCustomHeaders(Headers3)
                                     protocol = TCompactProtocol.TCompactProtocol(transport)
                                     client = LineService.Client(protocol)
                                     qr = client.getAuthQrcode(keeplogedln=1,systemName=nama1)
                                     link = "line://au/q/" + qr.verifier
                                     print(link)
                                     cl.sendMessage(msg.to,"Starting white true")
                                     cl.sendMessage(msg.to,"Except")
                                     cl.sendMessage(msg.to,str(link))
                                     Header3.update({'x-lpqs' : ' /api/v4/TalkService.do','X-line-Acces': qr.verifier})
                                     json.loads(requests.session().get('https://gd2.line.naver.jp/Q',headers=Headers3).txt)
                                     Headers3.update({'x-lpqs' : ' /api/v4p/rs'})
                                     transport = THttpClient.THttpClient('https://gd2.line.naver.jp/api/v4p/rs')
                                     transport.setCustomHeaders(Headers3)
                                     protocol = TCompactProtocol.TCompactProtocol(tramsport)
                                     cl = LineService.Client(protocol)
                                     req = LoginRequest()
                                     req.type = 1
                                     req.verifier = qr.verifier
                                     req.e2eeVersion = 1
                                     res = clien.loginZ(req)
                                     print('\n')
                                     print(res.authToken)
                                 else:
                                 	cl.sendMessage(msg.to, "The bot has been mmade with zeheaders")
